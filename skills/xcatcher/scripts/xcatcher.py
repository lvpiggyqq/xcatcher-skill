...
def required_secret(variable: str) -> str:
    ...
    if not value:
        raise SystemExit(f"{variable} must be supplied through the process environment")
    return value


def api_key() -> str:
    return required_secret("XCATCHER_API_KEY")


def task_token() -> str:
    return required_secret("XCATCHER_TASK_TOKEN")
...
def main() -> None:
    ...
    if args.command == "health":
        ...
        status, payload, _, headers = request(base, "GET", "/api/v1/me", key=api_key())
        emit(status, payload, headers)
        ...
        status, payload, _, headers = request(base, "GET", f"/api/v1/tasks?{query}", key=api_key())
        emit(status, payload, headers)
        ...
        status, payload, _, headers = request(base, "GET", f"/api/v1/tasks/{args.task_id}", key=api_key())
        emit(status, payload, headers)
    elif args.command == "results":
        query = urllib.parse.urlencode({"limit": max(1, min(200, args.limit)), "offset": max(0, args.offset)})
        status, payload, _, headers = request(
            base, "GET", f"/api/v1/tasks/{args.task_id}/results?{query}", key=api_key()
        )
        emit(status, payload, headers)
        ...
        status, payload, _, headers = request(
            base, "GET", f"/api/v1/x402/tasks/{args.task_id}", key=task_token()
        )
        emit(status, payload, headers)
    elif args.command == "direct-results":
        query = urllib.parse.urlencode({"limit": max(1, min(200, args.limit)), "offset": max(0, args.offset)})
        status, payload, _, headers = request(
            base, "GET", f"/api/v1/x402/tasks/{args.task_id}/results?{query}", key=task_token()
        )
        emit(status, payload, headers)
    elif args.command == "direct-download":
        output = args.output or Path(f"task_{args.task_id}.xlsx")
        status, payload, raw, headers = request(
            base, "GET", f"/api/v1/x402/tasks/{args.task_id}/download", key=task_token()
        )
        if status >= 400:
            emit(status, payload, headers)
        ...
        output = args.output or Path(f"task_{args.task_id}.xlsx")
        status, payload, raw, headers = request(
            base, "GET", f"/api/v1/tasks/{args.task_id}/download", key=api_key()
        )
        if status >= 400:
            emit(status, payload, headers)
        output.write_bytes(raw)
